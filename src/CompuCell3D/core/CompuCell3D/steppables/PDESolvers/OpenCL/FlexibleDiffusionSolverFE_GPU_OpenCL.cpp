...
void FlexibleDiffusionSolverFE_GPU_OpenCL::prepareSolverParams(Dim3D fieldDim, DiffusionData const &diffData){
	...
	localWorkSize[2]=std::min((unsigned int) (oclHelper->getMaxWorkGroupSize()/(BLOCK_SIZE*BLOCK_SIZE)),  h_solverParam.dimz);
...
void FlexibleDiffusionSolverFE_GPU_OpenCL::PrepareKernelParams()
{
	...
	if(latticeType==SQUARE_LATTICE){
		...
		err  = err | clSetKernelArg(kernel, 4, sizeof(float)*(localWorkSize[0]+2)*(localWorkSize[1]+2)*(localWorkSize[2]+2), NULL);
		err  = err | clSetKernelArg(kernel, 5, sizeof(unsigned char)*(localWorkSize[0]+2)*(localWorkSize[1]+2)*(localWorkSize[2]+2), NULL);
		err  = err | clSetKernelArg(kernel, 6, sizeof(float)*localWorkSize[0]*localWorkSize[1]*localWorkSize[2], NULL);
		...
		err  = err | clSetKernelArg(kernel, 4, sizeof(float)*(localWorkSize[0]+2)*(localWorkSize[1]+2), NULL);
		err  = err | clSetKernelArg(kernel, 5, sizeof(unsigned char)*(localWorkSize[0]+2)*(localWorkSize[1]+2), NULL);

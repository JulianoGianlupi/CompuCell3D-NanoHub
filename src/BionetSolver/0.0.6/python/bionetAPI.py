...
if version_number:
    ...
    if int(version_split[1])>=9:
        ...
        '''
        raise ImportError('BionetSolver/bionetAPI import error - deprecated module')
...
class BionetworkManager( object ):
    ...
    def loadSBMLModel( self, sbmlModelName, sbmlModelPath, modelKey = "", timeStepOfIntegration = -1.0 ):
        ...
        pathToLoad=sbmlModelPath
        try:
            f=open(pathToLoad,'r')
            f.close()
        except IOError, e:
            if self.simulator.getBasePath()!='':
                pathToLoad=os.path.join(self.simulator.getBasePath(),pathToLoad)
                
        newSBMLModel = BionetworkSBML( sbmlModelName, pathToLoad, timeStepOfIntegration )            
